...
const BestProducts = () => {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/api/products/?limit=10&desc=true')
  ...
  return (
    <div className="container">
      <h5 className="text-center text-uppercase">Best Selling Products</h5>
      <Swiper
        slidesPerView={4}
        spaceBetween={10}
        pagination={{
          ...
          <SwiperSlide key={product.id} style={{height:'30rem'}}>
            <Card className="p-2" style={{ width: '20rem',height:'100%', border: 'none' }}>
              ...
              <Link to={`/product/${product.id}`} className='text-decoration-none text-black'><p>{truncateText(product.name, 100)}</p></Link>
                ...
                <a href="#">
                  <Button className="btn btn-danger w-100" onClick={() => addToCart(product.id)}>Add To Cart</Button>
                </a>
              </Card.Body>
            </Card>
          </SwiperSlide>
